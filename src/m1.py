...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        return math.sqrt((point.x - self.circle.center.x) ** 2 +
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        c_index = random.randrange(0, len(self.colors))
        self.circle.fill_color = self.colors[c_index]
    ...
    def swallow(self, other_circle_changer):
        ...
        new_center = rg.Point(self.x + (1 / 2) * (other_circle_changer.x - self.x),
                              self.y + (1 / 2) * (other_circle_changer.y - self.y))
        new_radius = math.sqrt((self.x - new_center.x) ** 2 + (self.y - new_center.y) ** 2)
    ...
    def change_to_next_color_in_tuple(self):
        ...
        if self.color_changed < len(self.colors):
            self.color_changed = self.color_changed + 1
            self.circle.fill_color = self.colors[self.color_changed - 1]
        else:
